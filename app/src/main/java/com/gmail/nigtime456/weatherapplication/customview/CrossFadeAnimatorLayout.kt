...
class CrossFadeAnimatorLayout : FrameLayout {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        currentDisplayedChild =
            typedArray.getInteger(R.styleable.CrossFadeAnimatorLayout_cfalDisplayedChild, 0)
        startDelayDuration =
            typedArray.getInteger(
                R.styleable.CrossFadeAnimatorLayout_cfalStartDelay,
                DEFAULT_START_DELAY
            )
                .toLong()

        animationDuration = typedArray.getInteger(
            R.styleable.CrossFadeAnimatorLayout_cfalAnimationDuration,
            DEFAULT_DURATION
        ).toLong()
